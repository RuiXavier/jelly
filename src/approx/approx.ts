...
function callPre(mod: string, loc: string, base: any, fun: any, args: Array<any>, isNew: boolean): {proceed: boolean, result?: any} {
    if (fun === Function) {
        ...
        if (!error) {
            ...
            if (logger.isDebugEnabled())
                logger.debug(str);
            const result = fun(...funargs, transform(mod, loc, String(funbody)));
            hints.addEvalHint({
        ...
        if (fun.name === "require" && "resolve" in fun && "cache" in fun) { // probably a require function
            ...
            case eval:
                ...
                const result = fun(transform(mod, loc, str));
                return {proceed: false, result};
 ...
 */
function transform(mod: string, loc: string | undefined, code: string): string {
    const name = loc ? `${mod}:eval[${loc}]` : mod;
    instrumenting = true;
    try {
        const result = approxTransform(code, name, "commonjs", (level, msg) => logger.log(level, msg));
        if (!result)
            return "";
        if (!loc) // eval code doesn't count in numStaticFunctions
            numStaticFunctions += result.numStaticFunctions;
        for (const req of result.staticRequires)
            mapGetSet(staticModuleLoads.commonjs, mod).add(req);
...
function transformModule(filename: FilePath, code: string): string {
    ...
    if (!(typeof (filename as any) === "string" && typeof (code as any) === "string")) // value likely generated by the proxy, ignore
        ...
        transformed = `module.exports = ${PREFIX}proxy`;
    } else {
        try {
            checkFile(filename);
        } catch (err) {
        ...
        transformed = transform(filename, undefined, code); // using filename as module name, manager will convert
    }
    return transformed;
}
...
port1.on("message", (msg:
                         ...
                         {type: "metadata", numStaticFunctions: number, staticRequires: string[], filename: FilePath, codeSize: number}) => {
    switch (msg.type) {
        case "log": {
            const {level, str} = msg;
        ...
        case "metadata": {
            numStaticFunctions += msg.numStaticFunctions;
            totalCodeSize += msg.codeSize;
            for (const req of msg.staticRequires)
                mapGetSet(staticModuleLoads["module"], msg.filename).add(req);
            break;
        }
    }
});
...
(Module as any).prototype._compile = function(content: unknown, filename: unknown): any {
    ...
    if (!instrumenting) { // prevents instrumentation of Jelly runtime modules
        ...
        content = transformModule(filename, orig);
    }
    try {
